...
finance.command('upload-r1').description('Upload R1 XLSX file to ReturnPro staging').requiredOption('--file <path>', 'Path to R1 XLSX file').requiredOption('--user-id <uuid>', 'Supabase user UUID').requiredOption('--month <YYYY-MM>', 'Month in YYYY-MM format').action(async (opts: { file: string; userId: string; month: string }) => { if (!existsSync(opts.file)) { console.error(`File not found: ${opts.file}`); process.exit(1) } try { const result = await processR1Upload(opts.file, opts.userId, opts.month); console.log(`R1 upload complete: ${result.rowsInserted} rows inserted, ${result.rowsSkipped} skipped (${result.programGroupsFound} program groups)`); if (result.warnings.length > 0) { console.log(`Warnings: ${result.warnings.slice(0, 10).join(', ')}`) } } catch (err) { console.error(`R1 upload failed: ${err instanceof Error ? err.message : String(err)}`); process.exit(1) } })
