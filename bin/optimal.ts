...
import { validateTemplateFile, formatValidationReport } from '../lib/returnpro/template-validator.js'
import { assessCoverage, formatCoverageReport } from '../lib/returnpro/coverage.js'
...
finance.command('upload').description('Upload NetSuite CSV/XLSX to ReturnPro staging').requiredOption('--file <path>', 'Path to NetSuite file (CSV, XLSX, or XLSM)').requiredOption('--user-id <uuid>', 'Supabase user UUID').option('--strict-template', 'Abort if template formulas have critical Solution7 defects (NSGLADBAL typo, "New Program" filter, etc.)', false).option('--skip-validate', 'Skip the formula validator (default runs it on .xlsm files)', false).action(async (opts: { file: string; userId: string; strictTemplate: boolean; skipValidate: boolean }) => { if (!existsSync(opts.file)) { console.error(`File not found: ${opts.file}`); process.exit(1) } if (!opts.skipValidate && /\.xlsm$/i.test(opts.file)) { try { const v = await validateTemplateFile(opts.file); if (v.totalDefectiveCells > 0) { console.error(`\nTemplate formula validator: ${v.totalDefectiveCells}/${v.totalFormulaCells} formula cells defective`); for (const k of Object.keys(v.defectCountsByKind) as Array<keyof typeof v.defectCountsByKind>) { const n = v.defectCountsByKind[k]; if (n > 0) console.error(`  - ${k}: ${n}`) } if (v.hasCriticalDefects && opts.strictTemplate) { console.error('\nCritical defects detected — aborting upload (--strict-template). Run "optimal finance validate-template --file <path>" for details.'); process.exit(2) } else if (v.hasCriticalDefects) { console.error('\n⚠ Critical defects present — staging numbers will be wrong. Pass --strict-template to abort, or run "optimal finance validate-template" for details.\n') } } } catch (err) { console.error(`Template validator failed (continuing): ${err instanceof Error ? err.message : String(err)}`) } } try { const result = await processNetSuiteUpload(opts.file, opts.userId); console.log(`NetSuite upload: ${result.inserted} rows inserted (months: ${result.monthsCovered.join(', ')})`); if (result.warnings.length > 0) { console.log(`Warnings: ${result.warnings.slice(0, 10).join(', ')}`) } if (result.monthsCovered.length > 0) { try { const coverage = await assessCoverage(result.monthsCovered); console.log(''); console.log(formatCoverageReport(coverage)); if (coverage.hasCoverageGaps) console.log('\n⚠ Run `optimal finance audit --months <YYYY-MM>` for per-account match detail.') } catch (err) { console.error(`Coverage check failed (upload still succeeded): ${err instanceof Error ? err.message : String(err)}`) } } } catch (err) { console.error(`NetSuite upload failed: ${err instanceof Error ? err.message : String(err)}`); process.exit(1) } })
...
finance.command('coverage').description('Lightweight coverage check: staging vs confirmed_income_statements (account count + dollar totals per month)').option('--months <m>', 'YYYY-MM months (CSV or repeatable)', collectMonths, undefined as string[] | undefined).option('--threshold <pct>', 'Account coverage threshold below which a gap is flagged (0-1)', '0.90').option('--dollar-tol <frac>', 'Dollar tolerance fraction outside [1-tol, 1+tol] flags a gap', '0.10').action(async (opts: { months?: string[]; threshold: string; dollarTol: string }) => { try { const result = await assessCoverage(opts.months, { accountCoverageThreshold: parseFloat(opts.threshold), dollarToleranceFraction: parseFloat(opts.dollarTol) }); console.log(formatCoverageReport(result)); process.exit(result.hasCoverageGaps ? 1 : 0) } catch (err) { console.error(`Coverage check failed: ${err instanceof Error ? err.message : String(err)}`); process.exit(1) } })

finance.command('validate-template').description('Validate Solution7 NSGLAPBAL formulas in an XLSM template before upload').requiredOption('--file <path>', 'Path to NetSuite XLSM template').option('--sheet <name>', 'Worksheet name (default: Data Entry)').option('--samples <n>', 'Max samples to print', '20').option('--strict', 'Exit non-zero if any critical defects detected', false).action(async (opts: { file: string; sheet?: string; samples: string; strict: boolean }) => { if (!existsSync(opts.file)) { console.error(`File not found: ${opts.file}`); process.exit(1) } try { const result = await validateTemplateFile(opts.file, { sheetName: opts.sheet, sampleLimit: parseInt(opts.samples, 10) }); console.log(formatValidationReport(result)); if (opts.strict && result.hasCriticalDefects) process.exit(2) } catch (err) { console.error(`Validation failed: ${err instanceof Error ? err.message : String(err)}`); process.exit(1) } })
...
agent.command('coordinate').description('Run the coordinator loop').option('--interval <ms>', 'Poll interval in milliseconds', '30000').option('--max-agents <n>', 'Maximum agents to manage', '10').option('--stale-minutes <n>', 'Heartbeat-stale threshold in minutes', '15').option('--no-detect-stale', 'Disable heartbeat-based stale detection').option('--no-rebalance', 'Disable load-based rebalance').option('--no-auto-assign', 'Poll only; do not auto-claim ready tasks').action(async (opts) => { await runCoordinatorLoop({ pollIntervalMs: parseInt(opts.interval), maxAgents: parseInt(opts.maxAgents), staleHeartbeatMinutes: parseInt(opts.staleMinutes), detectStale: opts.detectStale !== false, rebalanceLoad: opts.rebalance !== false, autoAssign: opts.autoAssign !== false }) })
...
// admin command group removed — backing lib/admin/ modules deleted in cleanup
